...
// assign p_event_msg to ricEventTriggerDefinition
	...
	p_action->actionDefinition_formats.t=T_E2SM_KPM_ActionDefinition_actionDefinition_formats_actionDefinition_Format1;
        p_action->actionDefinition_formats.u.actionDefinition_Format1=rtxMemAllocTypeZ(&asn1_ctx, E2SM_KPM_ActionDefinition_Format1);	
	if(p_action->actionDefinition_formats.u.actionDefinition_Format1==NULL)
	{
		printf("=====Mem alloc failed !!========\n\n");
	}
	//asn1Print_E2SM_KPM_ActionDefinition("Action Definition",p_action);
        p_action->actionDefinition_formats.u.actionDefinition_Format1->granulPeriod=10000;
	p_action->ric_Style_Type=1;
	E2SM_KPM_MeasurementInfoItem              *meas=NULL;
	
	int      a1=0;
	E2SM_KPM_LabelInfoItem        *label=NULL;

	OSRTDListNode        *Node=NULL;
	OSRTDListNode        *Node1=NULL;
	Node=NULL;
        rtxDListInit(&(p_action->actionDefinition_formats.u.actionDefinition_Format1->measInfoList));
        asn1Init_e2ap_RANfunctionsID_List(&(p_action->actionDefinition_formats.u.actionDefinition_Format1->measInfoList));
        int loop =0 ;
	...
	{  
            ...
            memset(meas ,0,sizeof(E2SM_KPM_MeasurementInfoItem));
	    meas->measType.t=T_E2SM_KPM_MeasurementType_measID;
            if(loop==0)
	    {
	       rtxDListInit(&(meas->labelInfoList));
               asn1Init_e2ap_RANfunctionsID_List(&(meas->labelInfoList));
	    }
            switch(loop)
	    {
              case 0:
		meas->measType.u.measID=101;
            	rtxDListAllocNodeAndData(&asn1_ctx,
                  E2SM_KPM_LabelInfoItem,
                  &Node1,&label);
                  memset(label ,0,sizeof(E2SM_KPM_LabelInfoItem));
		  label->measLabel.m.noLabelPresent=1;
            	  rtxDListAppendNode (
                  &(meas->labelInfoList),
                  Node1);
		...
		meas->measType.u.measID=102;
		  ...
		  label->measLabel.m.noLabelPresent=1;
            ...
            rtxDListAppendNode (
                  &(p_action->actionDefinition_formats.u.actionDefinition_Format1->measInfoList),
                  Node);
        }
//	asn1Print_E2SM_KPM_MeasurementInfoItem("Meas_Info_List",meas);
...
// assign p_event_msg to ricEventTriggerDefinition
	...
	//asn1Print_E2SM_KPM_ActionDefinition("Action Definition",p_action);
        p_action->actionDefinition_formats.u.actionDefinition_Format1->granulPeriod=500;
	p_action->ric_Style_Type=1;
	E2SM_KPM_MeasurementInfoItem              *meas=NULL;
	...
	{  
	    ...
	    {
              case 0:
		meas->measType.u.measID=501;
	//	meas->measType.u.measName="RRU.PrbTotDlDist";
            	rtxDListAllocNodeAndData(&asn1_ctx,
                  E2SM_KPM_LabelInfoItem,
                  &Node1,&label);
                  memset(label ,0,sizeof(E2SM_KPM_LabelInfoItem));
		  label->measLabel.m.noLabelPresent=1;
              ...
              case 1:
		meas->measType.u.measID=102;
	//	meas->measType.u.measName="RRU.PrbTotUlDist";
            	rtxDListAllocNodeAndData(&asn1_ctx,
                  E2SM_KPM_LabelInfoItem,
                  &Node1,&label);
                  memset(label ,0,sizeof(E2SM_KPM_LabelInfoItem));
...
int main()    
{
    int Sockfd, connSock, connSock1,ret, in, flags, datalen;
  ...
  /** Create a socket **/
   ...
   // connSock1 = accept(Sockfd, (struct sockaddr*)NULL, (int*)NULL);
     ...
     while(1)
	{
		if(pp_asn_msg==NULL)
		{
		      ret=encode_e2_setup_response(&pp_asn_msg,&p_asn_msg_len,0);
        	}
   ...
   pp_asn_msg=NULL;
     while(1)
	{
		if(pp_asn_msg==NULL)
		{
		    ret=encode_e2_setup_response(&pp_asn_msg,&p_asn_msg_len,1);
        	}
//		ret = sctp_sendmsg(connSock1,pp_asn_msg,p_asn_msg_len, NULL, 0, 0, 0, 0, 0, 0);
   ...
   while(1)
    ...
    {
       	memset(octet_bitstring, 0, sizeof(UInt8));
       	in = sctp_recvmsg(connSock, octet_bitstring,8196,
                         NULL, 0, 0, 0);
       	if (in <= 0)
       	{
            	printf("Error in sctp_recvmsg\n");
       	...
       	memset(octet_bitstring, 0, sizeof(UInt8));
       	in = sctp_recvmsg(connSock1, octet_bitstring,8196,
                         NULL, 0, 0, 0);
       	if (in <= 0)
       	{
            	printf("Error in sctp_recvmsg\n");
       	...
       	{
      		printf("Length of Data received1: %d\n", in);
      		printf("Data1 : ");  // Modify how you print the octet bitstring
       		for ( i = 0; i < in; i++)
       		{
              		printf("%02X ", octet_bitstring[i]);
      		}
